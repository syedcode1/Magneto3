    ...
    foreach ($technique in $aptTechniques) {
        ...
        if ($technique.ValidationRequired) {
            ...
            if (-not $validationResult) {
                Write-Host "[$($technique.ID)] Skipping - validation failed" -ForegroundColor Yellow
                $canExecute = $false
            }
        }
...
if ($APTCampaign) {
    ...
    foreach ($tech in $selectedTechniques) {
        ...
        if ($tech.ValidationRequired) {
            ...
            if (-not $validationResult) {
                Write-Host "[$($tech.ID)] Skipping - validation failed" -ForegroundColor Yellow
...
try {
    ...
    Write-Host "Could not open log file automatically. Please open manually: $logFile" -ForegroundColor Yellow
}
