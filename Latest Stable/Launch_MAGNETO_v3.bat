...
echo.
echo  KEY FEATURES:
echo  [+] 55 MITRE ATT^&CK Techniques
echo  [+] 7 APT Campaign Simulations
echo  [+] 10 Industry Vertical Scenarios
echo  [+] MITRE ATT^&CK Tactics Visualization
echo  [+] NIST 800-53 Rev 5 Control Mapping
echo  [+] Enhanced Threat Intelligence
echo  [+] Real-World APT Attribution
echo  [+] HTML Attack Reports with Compliance Data
echo.
...
REM Create folder structure if it doesn't exist
if not exist "%~dp0Logs" (
    echo Creating Logs directory...
    mkdir "%~dp0Logs"
)

if not exist "%~dp0Logs\Attack Logs" (
    echo Creating Attack Logs directory...
    mkdir "%~dp0Logs\Attack Logs"
)

if not exist "%~dp0Logs\GUI Logs" (
    echo Creating GUI Logs directory...
    mkdir "%~dp0Logs\GUI Logs"
)

if not exist "%~dp0Reports" (
...
if %EXIT_CODE% EQU 0 (
    ...
    echo   1. Check logs in Logs\GUI Logs folder
